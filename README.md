...
- ノート本文を大きなカードとして表示
- 2Hop Links Plus の本文内関連領域と Obsidian 標準の文書内バックリンクを、本文カード色ではなく周囲の canvas 色で区切って表示
- 幅の小さいリンク先プレビューでは内側のカード枠を外し、通常ノートカードと同じ背景・文字色、外周だけの均一な4px角丸リングで内容幅を確保
...
標準のページプレビューでは、通常ノートカードと同じ背景・文字色、外周だけの均一な4px角丸リングが適用されます。任意のHover Editorプラグインを導入している場合は、アクティブ、非アクティブ、ピン留め状態の上部バーにも補助スタイルが適用されます。Hover Editorは必須ではありません。
...
### 2Hop Links Plus integration

2Hop Links Plus が提供する`.twohop-links-container--inline`と
`.is-document-related-region`を使い、本文内 2-hop 領域を本文カードとは異なる
canvas 面として表示します。Obsidian 標準の文書内バックリンクも同じ面へ揃えます。

CSS は Obsidian／CodeMirror の要素を移動せず、背景・余白・文字色だけを変更します。
Live Preview／Sourceでは、親カードの1px枠が関連領域の外側へ残らないようcanvas色で
覆います。2Hopが専用領域へ表示する一時ソートメニューだけは、ライトでは白地に黒系、
ダークでは暗色地に明色となるようテーマ色を引き継ぎます。ほかのObsidianメニューには
適用しません。検索、設定、ソート、カード列数など2Hop内部の操作と配置はプラグイン側が
担当します。
